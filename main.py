...
async def _download_video_inner(client: Client, query, link: str, quality: str, status_msg, chat_id: int):
    if True:
        ...
        try:
            ...
            video_info = await asyncio.to_thread(fetch_diskwala_video, link, auth)
            ...
            # NOTE: requests + r.iter_content() are fully synchronous/blocking
            # calls. Running them directly on this coroutine would freeze the
            # ENTIRE process's asyncio event loop for the whole download
            # (main bot + every Titanium clone all share one loop) — that's
            # exactly why big/slow downloads used to make every button on the
            # main bot look "dead" until the file finished. The actual
            # network + disk I/O now runs in a background thread via
            # asyncio.to_thread(); this coroutine just polls a shared dict
            # once a second to update the progress message, so the event
            ...
            def _blocking_download():
                try:
                    ...
                    _progress["total"] = int(resp.headers.get("content-length", 0))
