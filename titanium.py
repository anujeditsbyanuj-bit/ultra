...
def _panel_text(bots) -> str:
    ...
    # Multi-bot view — lists every connected clone, not just the first one.
    lines = ["⚡ <b>TITANIUM CLONE MODE</b>", "", f"You have {len(bots)} bot(s) connected:", ""]
    for b in bots:
        try:
            token = b["token"]
            username = b["username"]
            ...
            src = "🤖 MANAGED" if b.get("source") == "managed" else "🔑 MANUAL"
            lines.append(f"• <b>@{username}</b> — {status} ({src})")
        except Exception as e:
    ...
    lines += [
        "",
        "Each clone bot handles downloads independently, preventing "
        "floodwait on the main bot. Tap a bot below to manage it.",
    ]
    return "\n".join(lines).rstrip()
...
def _panel_buttons(bots, make_button, BTN_PRIMARY, BTN_DANGER):
    ...
    # One row per connected bot: Status + Disable, so every clone is
    # reachable from the panel, not just bots[0].
    rows = []
    for b in bots:
        try:
            username = b["username"]
            rows.append([
                make_button(f"📊 @{username}", callback_data=f"titanium_details:{username}", style=BTN_PRIMARY),
                make_button("🔄", callback_data=f"titanium_replace:{username}", style=BTN_PRIMARY),
                make_button("❌", callback_data=f"titanium_disable_confirm:{username}", style=BTN_DANGER),
            ])
        except Exception as e:
            # Same corrupt-entry guard as _panel_text — skip just this
            # bot's row instead of crashing the whole keyboard build.
            logger.warning(f"Titanium panel buttons: skipping corrupt bot entry {b!r}: {e}")
            continue

    rows.append([
        make_button("➕ Add Bot", callback_data="titanium_addbot", style=BTN_PRIMARY),
        make_button("🏓 Ping All", callback_data="titanium_ping", style=BTN_PRIMARY),
    ])
    if _managed_bots_available():
        rows.append([make_button("🤖 Auto-Create Bot", callback_data="titanium_autocreate", style=BTN_PRIMARY)])
...
def register_titanium_handlers(app, *, make_button, BTN_PRIMARY, BTN_DANGER, SC, ParseMode,
    ...
    async def _titanium_ping_cb(client, query: CallbackQuery):
        ...
        # Result shown as a popup alert (query.answer(..., show_alert=True))
        # to match the reference project — NOT an edited message. A
        # callback query can only be answered once, so unlike the old
        # "answer immediately, edit later" version, everything below runs
        # before the single answer() call. Pings against an already-warm
        # clone are near-instant, so this stays well inside the callback
        # query's validity window in practice.
        try:
            owner_id = query.from_user.id
            bots = await get_titanium_bots(owner_id)
            if not bots:
            ...
            try:
                await query.answer(f"Ping failed: {e}", show_alert=True)
            except Exception:
                pass
