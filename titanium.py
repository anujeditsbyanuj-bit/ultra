...
def register_titanium_handlers(app, *, make_button, BTN_PRIMARY, BTN_DANGER, SC, ParseMode,
    ...
    async def _titanium_ping_cb(client, query: CallbackQuery):
        # Answered inside a try/except so a real error always reaches the
        # user as a visible alert instead of the button just silently
        # doing nothing (previously: any exception here — bad token, DB
        # hiccup, clone start failure — meant query.answer() never ran,
        # and Telegram just shows the tap as a no-op with zero feedback).
        try:
            owner_id = query.from_user.id
            bots = await get_titanium_bots(owner_id)
            if not bots:
            ...
            try:
                await query.answer(f"⚠️ Ping failed: {e}", show_alert=True)
            except Exception:
                pass
