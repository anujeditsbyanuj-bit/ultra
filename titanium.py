...
def _panel_text(bots) -> str:
    ...
    # Single-primary-bot view — matches the reference project's panel
    # layout exactly (BOT / SOURCE / STATUS, then the STATUS/PING/REPLACE/
    # AUTO-CREATE/DISABLE/BACK button set below in _panel_buttons).
    b = bots[0]
...
def _panel_buttons(bots, make_button, BTN_PRIMARY, BTN_DANGER):
    ...
    rows = [
        [
            make_button("📊 Status", callback_data=f"titanium_details:{username}", style=BTN_PRIMARY),
            make_button("🏓 Ping", callback_data="titanium_ping", style=BTN_PRIMARY),
        ],
        [make_button("🔄 Replace Bot", callback_data=f"titanium_replace:{username}", style=BTN_PRIMARY)],
    ]
    if _managed_bots_available():
        rows.append([make_button("🤖 Auto-Create Bot", callback_data="titanium_autocreate", style=BTN_PRIMARY)])
    rows.append([make_button("❌ Disable Titanium", callback_data=f"titanium_disable_confirm:{username}", style=BTN_DANGER)])
